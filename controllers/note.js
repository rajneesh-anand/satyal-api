...
exports.deleteNoteFromOnlineClass=async (req, res) => {
  try{
      ...
      if(!id || !onlineClassId || !teacherEmail){
        return(
        res.status(400).type('json').json({error:"Bad Request, Please Try Again"}))
      }
      ...
      if(!onlineClassResponse){
        return res.status(400).type('json').json({error:"Bad Request, Please Try Again"})
      }
      // authenticating for authorize user
      if(onlineClassResponse?.teacherEmail !== teacherEmail){
        return res.status(401).type('json').json({error:"!You Are An Unauthorized User"})
      }
      ...
      if(!onlineClassNoticeDeleteResponse){
         return res.status(500).type('json').json({error: "This Notice Does Not Delete, Please Try Again"})
      }  

      return(
        res.status(204).type('json')
        .json({message : "Notice Is Deleted"})
      )  
  }catch(err){
    res
    .status(500)
    .type('json')
    .json({error:"Internel Server Error"})
  }
}
